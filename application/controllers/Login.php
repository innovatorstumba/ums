...
class Login extends CI_Controller
{
    ...
    function index(){
        if($this->input->post('username') != "" && $this->input->post('password') != "")
        {
            ...
            $results = $this->LoginModel->validateLogin($uname, $pass);
            if ($results){
                $row = $results->row();
                if ($row->roles_name == "Family"){
                    //Family Leader Session Data
                    $ldr_data = $this->LoginModel->selectLeaderByUserId($row->usr_id);
                    ...
                    $adm_data = $this->LoginModel->selectAdminByUserId($row->usr_id);
